...
const templatingWithMustache = (() => {
  ...
  characters.forEach(object => {
    ...
    return allCharacters.push(html);
  });
...
const getCourseKeys = ((obj) => {
  // Solution code here...
  let arr = [];
  let objectPieces = Object.keys(obj);
  objectPieces.forEach(objectPieces => {
    arr.push(objectPieces);
  });
  return arr;
});
...
const getHouses = ((arr) => {
  let houses = [];
  // Solution code here...
  arr.forEach(allHouses => {
    houses.push(allHouses.house);
  });
  return houses;
});
...
const hasChildrenValues = ((arr, character) => {
  ...
  let kids = false;
  arr.forEach((object) => {
    if (character === object.name) {
      Object.values(object)[2].length > 0 ? kids = true : kids = false;
    }
  });
  return kids;
});
