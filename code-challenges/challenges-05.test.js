...
const templateWithJQuery = (() => {
  ...
  starWarsPeople.forEach(character => {
    ...
    let templateTwo = $(`<section>${template}</section>`);

    templateTwo.find('h2').text(character.name);
    templateTwo.find('h3').text(character.height);
    templateTwo.find('p').text(character.eye_color);

    $('main').append(templateTwo);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    let littleOne = item.slice(1);
    let index = (littleOne.indexOf(' ', 2) + 1);
    result.push(littleOne.slice(index));
  });
  return result;
};
